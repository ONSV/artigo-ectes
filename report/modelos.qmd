...
```{r}
metrix(data = sim_pred, estimate = .pred_class, truth = motociclista) |> kable()
conf_mat(sim_pred, truth = motociclista, estimate = .pred_class)
```

## Modelo Cross LogReg

```{r}
data_folds <- vfold_cv(sim_ajustado)

log_10fold <- fit_resamples(object = log_wflow, resamples = data_folds)

log_10fold |> collect_metrics(summarize = FALSE) |> kable()
```